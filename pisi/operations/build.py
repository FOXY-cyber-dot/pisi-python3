...
def exclude_special_files(filepath, fileinfo, ag):
    ...
    if "libtool" in keeplist:
        ...
        if re.match(patterns["libtool"], fileinfo) and \
            ...
            new_ladata = re.sub(r"%s/\S*/install/" % ctx.config.tmp_dir(), "/",
                                new_ladata)
        ...
        if not os.path.exists(src_dir):
            ...
            while not os.path.exists(src_dir):
                ...
                if not ext:
                    break
            if not os.path.exists(src_dir):
                src_dir = util.join_path(self.pkg_work_dir(), [d for d in next(os.walk(self.pkg_work_dir()))[1] if not d.startswith(".")][0])
